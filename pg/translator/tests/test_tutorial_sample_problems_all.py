...
_pg_root = Path(__file__).parent.parent.parent.parent
...
def get_all_problem_files() -> List[Path]:
    ...
    Returns:
        ...
        Returns empty list if directory doesn't exist or no files found.
    """
...
def pytest_generate_tests(metafunc):
    """Dynamically parametrize test_problem_renders with all problem files."""
    if "problem_path" in metafunc.fixturenames:
        problem_files = get_all_problem_files()
        if not problem_files:
            # Skip the test if no problem files found
            pytest.skip(
                "No .pg files found in tutorial/sample-problems", allow_module_level=True)
        metafunc.parametrize(
            "problem_path",
            problem_files,
            ids=lambda p: str(p.relative_to(
                _pg_root / "tutorial" / "sample-problems")).replace("\\", "/")
        )
...
class TestTutorialSampleProblems:
    ...
    @staticmethod
    def get_all_problem_files() -> List[Path]:
        """Wrapper for module-level get_all_problem_files()."""
        return get_all_problem_files()
    ...
    def get_category_and_name(problem_path: Path) -> Tuple[str, str]:
        ...
        parts = problem_path.relative_to(
            problem_path.parent.parent.parent.parent).parts
        if len(parts) >= 3:
            category = parts[2]
    ...
    def test_all_sample_problems_render(self):
        ...
        print(f"\n{'='*80}")
        print(
            f"Results: {passed} passed, {failed} failed out of {total} total")
        print(f"Success rate: {passed/total*100:.1f}%")
        print(f"{'='*80}\n")

        # Fail the test if any problems have errors
        assert len(
            failures) == 0, f"{len(failures)} problems failed to render:\n" + "\n".join(failures)
    ...
    def test_problem_renders(self, problem_path):
        ...
        assert isinstance(result.statement_html,
                          str), "statement_html should be a string"
        ...
        if not is_snippet:
            assert len(
                result.statement_html) > 0, "statement_html should not be empty"
    ...
    def test_sample_problems_directory_exists(self):
        ...
        ), f"tutorial/sample-problems directory not found at {tutorial_dir}"
    ...
    def test_sample_problems_contain_files(self):
        ...
        problem_files = self.get_all_problem_files()
        assert len(
            problem_files) > 0, "No .pg files found in tutorial/sample-problems"
        print(
...
def sample_problems_stats(problem_files):
    ...
    categories = {}
    for problem_path in problem_files:
        category, _ = TestTutorialSampleProblems.get_category_and_name(
            problem_path)

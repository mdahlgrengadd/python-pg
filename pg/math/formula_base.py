...
class FormulaBase(MathValue):
    """
    FormulaBase represents a mathematical expression with deferred evaluation.

    Unlike numeric types (Real, Complex) which have immediate values, FormulaBase
    ...
    Examples:
        >>> f = FormulaBase("x^2 + 2*x + 1")
        >>> f.eval(x=3)  # Returns Real(16)
        >>> f.diff("x")  # Returns FormulaBase("2*x + 2")
        >>> f.substitute("x", Real(5))  # Returns FormulaBase("36")
    """
    ...
    def reduce(self) -> FormulaBase:
        ...
        if self._sympy_expr is not None:
            ...
            return FormulaBase(simplified, self.variables, self.context)
        else:
            # No simplification without SymPy
            return self

    def substitute(self, **substitutions) -> 'FormulaBase':
        """
        ...
        Returns:
            New FormulaBase with substitutions applied

        Example:
            >>> f = FormulaBase("x^2 + y", ['x', 'y'])
            >>> f.substitute(x='2*t', y='t+1')
            FormulaBase("4*t^2 + t + 1")
        """
        if self._sympy_expr is not None:
            subs_dict = {}
            for var, expr in substitutions.items():
                ...
                elif isinstance(expr, FormulaBase):
    ...
    def diff(self, var: str) -> FormulaBase:
        ...
        if self._sympy_expr is not None:
            ...
            return FormulaBase(derivative, self.variables, self.context)
        else:
            raise RuntimeError("Cannot differentiate: expression not parsed")

    def D(self, var: str) -> 'FormulaBase':
    ...
    def integrate(self, var: str) -> FormulaBase:
        """
        ...
        if self._sympy_expr is not None:
            ...
            return FormulaBase(integral, self.variables, self.context)
        else:
    ...
    def __add__(self, other: Any) -> Formula:
        ...
        if SYMPY_AVAILABLE and self._sympy_expr is not None:
            ...
            return FormulaBase(result, combined_vars, self.context)
        else:
            # Fallback: string concatenation
            return FormulaBase(f"({self.to_string()}) + ({other.to_string()})", self.variables, self.context)

    def __radd__(self, other: Any) -> FormulaBase:
        """Right addition: other + self"""
        return self.__add__(other)

    def __sub__(self, other: Any) -> FormulaBase:
        ...
        if SYMPY_AVAILABLE and self._sympy_expr is not None:
            if isinstance(other, FormulaBase) and other._sympy_expr is not None:
                result = self._sympy_expr - other._sympy_expr
            else:
                result = self._sympy_expr - other.to_python()
    ...
    def __rsub__(self, other: Any) -> Formula:
        ...
        return FormulaBase.from_math_value(other).__sub__(self)

    def __mul__(self, other: Any) -> FormulaBase:
    ...
    def __rmul__(self, other: Any) -> FormulaBase:
        """Right multiplication: other * self"""
        return self.__mul__(other)

    def __truediv__(self, other: Any) -> FormulaBase:
        """Division: self / other"""
        if not isinstance(other, MathValue):
            from .numeric import Real
    ...
    def __rtruediv__(self, other: Any) -> FormulaBase:
        ...
        return FormulaBase.from_math_value(other).__truediv__(self)

    def __pow__(self, other: Any) -> FormulaBase:
    ...
    def __rpow__(self, other: Any) -> FormulaBase:
        ...
        return FormulaBase.from_math_value(other).__pow__(self)

    def __neg__(self) -> FormulaBase:
        """Unary negation: -self"""
        if SYMPY_AVAILABLE and self._sympy_expr is not None:
            return FormulaBase(-self._sympy_expr, self.variables, self.context)
        else:
            return FormulaBase(f"-({self.to_string()})", self.variables, self.context)

    def __pos__(self) -> FormulaBase:
        """Unary positive: +self"""
        return self

    def __abs__(self) -> FormulaBase:
        """Absolute value: abs(self)"""
        if SYMPY_AVAILABLE and self._sympy_expr is not None:
            return FormulaBase(sp.Abs(self._sympy_expr), self.variables, self.context)
        else:
            return FormulaBase(f"abs({self.to_string()})", self.variables, self.context)

    @classmethod
    def from_math_value(cls, value: MathValue) -> FormulaBase:
        """Convert a MathValue to a FormulaBase."""
